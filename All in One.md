...
# CompTIA Security+ (SY0-701) Study Notes
    ...
    |---------|-----------|----------------|
    | CIA | Confidentiality, Integrity, Availability | Keep data secret, accurate, and accessible |
    | AAA | Authentication, Authorization, Accounting | Prove who you are, what you can do, and track it |
    | DAC | Discretionary Access Control | Owner controls who accesses their files |
    | MAC | Mandatory Access Control | System enforces access based on labels/clearance |
    | RBAC | Role-Based Access Control | Access based on your job role |
    | ABAC | Attribute-Based Access Control | Access based on multiple attributes/conditions |
    ...
    | Acronym | Full Name | Simple Meaning |
    |---------|-----------|----------------|
    | PICERL | Prepare, Identify, Contain, Eradicate, Recover, Lessons Learned | The 6 steps to handle a security incident |
    | SIEM | Security Information and Event Management | Tool that collects and analyzes security logs |
    | SOC | Security Operations Center | Team that monitors security 24/7 |
    | IOC | Indicator of Compromise | Clue that a system has been compromised |
    ...
    | Acronym | Full Name | Simple Meaning |
    |---------|-----------|----------------|
    | DMZ | Demilitarized Zone | Buffer zone between internet and internal network |
    | NAC | Network Access Control | Controls which devices can join the network |
    | VLAN | Virtual Local Area Network | Logically separates network segments |
    | VPN | Virtual Private Network | Encrypted tunnel over the internet |
    | IDS | Intrusion Detection System | Detects attacks and alerts you |
    | IPS | Intrusion Prevention System | Detects AND blocks attacks automatically |
    | NGFW | Next-Generation Firewall | Smart firewall that inspects deep packet content |
    ...
    | SDN | Software Defined Networking | Software controls network hardware centrally |
    ...
    | Acronym | Full Name | Simple Meaning |
    |---------|-----------|----------------|
    | AES | Advanced Encryption Standard | Fast, strong symmetric encryption standard |
    | RSA | Rivest–Shamir–Adleman | Asymmetric encryption using two keys |
    | ECC | Elliptic Curve Cryptography | Asymmetric encryption using smaller, efficient keys |
    | DES | Data Encryption Standard | Old weak symmetric encryption (avoid) |
    | SHA | Secure Hash Algorithm | Turns data into a fixed-length hash |
    | MD5 | Message Digest 5 | Old hashing algorithm, now considered broken |
    | PKI | Public Key Infrastructure | System for managing digital certificates and keys |
    | CA | Certificate Authority | Trusted authority that issues certificates |
    | CSR | Certificate Signing Request | Request sent to CA to get a certificate |
    | CRL | Certificate Revocation List | List of revoked/invalid certificates |
    | OCSP | Online Certificate Status Protocol | Real-time check if a certificate is still valid |
    | TLS | Transport Layer Security | Encrypts data in transit (replaces SSL) |
    | SSL | Secure Sockets Layer | Old encryption protocol, now deprecated |
    | PGP | Pretty Good Privacy | Encrypts emails and files end-to-end |
    ...
    | Acronym | Full Name | Simple Meaning |
    |---------|-----------|----------------|
    | MFA | Multi-Factor Authentication | Login requires 2+ verification methods |
    | SSO | Single Sign-On | One login gives access to many systems |
    | SAML | Security Assertion Markup Language | Standard for passing login info between systems |
    | LDAP | Lightweight Directory Access Protocol | Protocol to look up users in a directory |
    | RADIUS | Remote Authentication Dial-In User Service | Centralizes authentication for network access |
    | TACACS+ | Terminal Access Controller Access-Control System Plus | Like RADIUS but separates AAA functions |
    | PAM | Privileged Access Management | Controls and monitors admin/privileged accounts |
    | IAM | Identity and Access Management | System for managing user identities and permissions |

    ---

    ## ⚠️ Vulnerabilities & Attacks

    | Acronym | Full Name | Simple Meaning |
    |---------|-----------|----------------|
    | CVE | Common Vulnerabilities and Exposures | Standardized ID number for known vulnerabilities |
    | CVSS | Common Vulnerability Scoring System | Score from 0–10 rating vulnerability severity |
    | XSS | Cross-Site Scripting | Injecting malicious scripts into websites |
    | CSRF | Cross-Site Request Forgery | Tricks user into making unwanted web requests |
    | SQLi | SQL Injection | Injecting SQL code to manipulate a database |
    | MitM | Man-in-the-Middle | Attacker secretly intercepts communications |
    | DoS | Denial of Service | Flood one system to make it unavailable |
    | DDoS | Distributed Denial of Service | Flood from many systems simultaneously |
    | APT | Advanced Persistent Threat | Long-term stealthy attack by skilled adversary |
    | RAT | Remote Access Trojan | Malware giving attacker remote control |
    | PII | Personally Identifiable Information | Personal data that can identify someone |
    ...
    | NIST | National Institute of Standards and Technology | US agency that creates security standards/frameworks |
    | SOX | Sarbanes-Oxley Act | Financial reporting rules for public companies |
    | FedRAMP | Federal Risk and Authorization Management Program | Cloud security standard for US government |
    | GRC | Governance, Risk, and Compliance | Managing security goals, risks, and rules together |
    ...
    | Acronym | Full Name | Simple Meaning |
    |---------|-----------|----------------|
    | BCP | Business Continuity Plan | Plan to keep business running during a disaster |
    | DRP | Disaster Recovery Plan | Plan to restore IT systems after a disaster |
    | RTO | Recovery Time Objective | Max time allowed to restore a system |
    | RPO | Recovery Point Objective | Max amount of data loss acceptable |
    ...
    | Acronym | Full Name | Simple Meaning |
    |---------|-----------|----------------|
    | DLP | Data Loss Prevention | Prevents sensitive data from leaving the org |
    | EDR | Endpoint Detection and Response | Monitors and responds to threats on endpoints |
    | XDR | Extended Detection and Response | EDR extended across network, cloud, and email |
    | UEBA | User and Entity Behavior Analytics | Detects unusual user/device behavior patterns |
    | NDA | Non-Disclosure Agreement | Legal agreement to keep information secret |
    | SLA | Service Level Agreement | Agreed service performance standards/guarantees |
    ...
    | Acronym | Full Name | Simple Meaning |
    |---------|-----------|----------------|
    | IaaS | Infrastructure as a Service | Rent servers and infrastructure from cloud |
    | PaaS | Platform as a Service | Rent platform to build/deploy apps on cloud |
    | SaaS | Software as a Service | Use software hosted entirely in the cloud |
    | CSP | Cloud Service Provider | Company providing cloud services (AWS, Azure) |
    | CASB | Cloud Access Security Broker | Security middleman between users and cloud apps |
    | SDP | Software Defined Perimeter | Hides network resources until identity is verified |
...
| 2 | Fundamentals of Security | [Fundamentals of Security](#fundamentals-of-security) |
| 3 | Threat Actors | [Threat Actors](#threat-actors) |
| 4 | Physical Security | [Physical Security](#physical-security) |
| 5 | Social Engineering | [Social Engineering](#social-engineering) |
| 6 | Malware | [Malware](#malware) |
| 7 | Data Protection | [Data Protection](#data-protection) |
| 8 | Cryptographic Solutions | [Cryptographic Solutions](#cryptographic-solutions) |
| 9 | Risk Management | [Risk Management](#risk-management) |
| 10 | Third-party Vendor Risks | [Third-party Vendor Risks](#third-party-vendor-risks) |
-----------------------------------------------------
| 11 | Governance and Compliance | [Governance and Compliance](#governance-and-compliance) |
| 12 | Asset and Change Management | [Asset and Change Management](#asset-and-change-management) |
| 13 | Audits and Assessments | [Audits and Assessments](#audits-and-assessments) |
| 14 | Cyber Resilience and Redundancy | [Cyber Resilience and Redundancy](#cyber-resilience-and-redundancy) |
| 15 | Security Architecture | [Security Architecture](#security-architecture) |
| 16 | Network Security | [Network Security](#network-security) |
| 17 | Identity and Access Management | [Identity and Access Management](#identity-and-access-management) |
| 18 | Incident Response | [Incident Response](#incident-response) |
| 19 | Vulnerability Management | [Vulnerability Management](#vulnerability-management) |
| 20 | Alerting and Monitoring | [Alerting and Monitoring](#Alerting-and-Monitoring) |
| 21 | Security Awareness | [Security Awareness](#Security-Awareness)|
...
-   *Availability*
    ...
    - **99.9% (Three Nines):** Allows for ~9 hours of downtime per year.
    - **99.999% (Five Nines):** Allows for only ~5 minutes of downtime per year.
-   *Redundancy*
...
-   Zero Trust demands verification for every device, user, and transaction within the network, regardless of its origin
    -   To create a zero trust architecture, we need to use two different planes
        -   **Control Plane**
            -   The Control Plane is responsible for the "thinking." It processes information to determine the best path for data to take.
        ...
        -   **Data Plane**
            -   The Data Plane is responsible for the "doing." It handles the actual transit of traffic through the device.            -   -    Data plane consists of the following
                -   *Subject/System*
                ...
                -   *Policy Enforcement Point*
                    ...
                    - is the "bridge"—it sits in the Data Plane to block traffic, but it listens to the Control Plane for instructions.

                    These components form the logical architecture of **Zero Trust Architecture (ZTA)** and **Attribute-Based Access Control (ABAC)**. For the SY0-701 exam, it is crucial to understand how these pieces interact to make "just-in-time" access decisions.

        ### **Access Control Policy Architecture**

        | Component | Role | Analogy: The Nightclub |
        | :--- | :--- | :--- |
        | **Subject / System** | The entity (user, device, or app) requesting access to a resource. | The **Patron** trying to get inside the club. |
        | **Policy Administrator (PA)** | The management interface used to create, edit, and delete security policies. | The **Club Owner** who writes the dress code and guest list. |
        | **Policy Engine (PE)** | The "brain" that evaluates the request against the PA's policies and external data (like time of day or location). | The **Head of Security** who checks the list and the ID to decide if the person fits the rules. |
        | **Policy Enforcement Point (PEP)** | The gatekeeper that sits in the data path and physically blocks or allows the traffic based on the PE's command. | The **Bouncer** standing at the door who actually lets you in or pushes you back. |

        ---

        ### **How the Workflow Functions**

        1.  **Request:** The **Subject** attempts to reach a resource (like a database or cloud app).
        2.  **Interception:** The **PEP** intercepts the request and asks the "Control Plane" for a decision.
        3.  **Evaluation:** The **Policy Engine** looks at the policies set by the **Policy Administrator** and determines if the Subject meets the criteria.
        4.  **Enforcement:** The PE sends the "Grant" or "Deny" command back to the **PEP**, which then opens or closes the connection.



        ---

        ### **Exam Tip: The Policy Decision Point (PDP)**
        In some documentation or exam questions, you may see the **Policy Engine** and **Policy Administrator** grouped together as the **Policy Decision Point (PDP)**. 

        * **PDP (Decision):** "Should they be allowed?"
        * **PEP (Enforcement):** "Open the door."

        Separating these ensures that even if a "bouncer" (PEP) is compromised, they don't have the authority to change the "rules" (Policies)—they only follow the commands of the central brain.
...
-   2 Most Basic Attributes of a Threat Actor
    ...
    -   *External Threat Actors*
        ...
        OBJ: 2.1 - **Capability** pertains to a threat actor's proficiency in devising new exploit techniques and tools. It can range from using commonly found attack tools to creating zero-day exploits in various systems. Those with the highest capabilities can even deploy non-cyber tools, such as political or military assets. While funding can boost a threat actor's capabilities by providing them the means to acquire resources, it doesn't specifically denote their expertise in developing unique exploits. **Sophistication** relates to the level of intricacy and advancement of a threat actor's methods and tools, but does not directly address their skill in crafting novel exploits. While resources can aid in bolstering a threat actor's capabilities, this term primarily refers to the tools and personnel that a threat actor can access or utilize. 
...
-   Money, not other motivations is the objective of their attacks even if the attack takes place in the political sphere
    ...
    -   **criminal syndicate**
        -   OBJ: 2.1 - Large organized crime rings have the financial means to hire and maintain a team of skilled individuals for sophisticated cyber operations.
...
-   To help prevent phishing your organization should regularly conduct user security awareness training that contains coverage of the various phishing techniques
    ...
    -   Mitigation
        ...
        **Technical Controls**: Using spam filters, digital signatures (S/MIME), and Multi-Factor Authentication (MFA).

        S/MIME (Secure/Multipurpose Internet Mail Extensions) digital signatures provide email authentication, data integrity, and non-repudiation by attaching a unique cryptographic fingerprint to messages, ensuring sender identity and that content hasn't been altered.
...
-   Some of the common other social engineering attacks
        ...
        -   *Piggybacking and Tailgating*
            ...
            -   *Evil Twin*    
                - Evil Twin is defined as a specific type of malicious rogue access point (AP) that impersonates a legitimate Wi-Fi network. 

                * Definition: A fraudulent Wi-Fi access point that uses the same SSID (Service Set Identifier) as a trusted network to trick users into connecting.
                * Primary Goal: To perform a Man-in-the-Middle (MitM) attack, allowing the attacker to intercept traffic, harvest credentials, or inject malware.
                * Common Tactic: Attackers often use a fake captive portal (a spoofed login page) to steal usernames and passwords.
                * Signal Strength: Attackers may use high-gain antennas or devices like a [Wi-Fi Pineapple](https://usa.kaspersky.com/resource-center/preemptive-safety/evil-twin-attacks) to provide a stronger signal than the legitimate AP, causing devices to connect to it automatically. [1, 2, 3, 4, 5, 6, 7] 
                * 

                Evil Twin vs. Rogue Access Point
                While often used interchangeably, the Security+ exam distinguishes them by intent and method:

                * Rogue AP: Any unauthorized access point added to a network. It could be an employee plugging in a personal router for convenience (no malicious intent).
                * Evil Twin: A deliberately malicious rogue AP designed specifically to mimic an existing, trusted network. [8, 9, 10, 11, 12] 
                * 

                Exam-Relevant Mitigations

                | Mitigation [1, 2, 13, 14, 15] | How it Works |
                |---|---|
                | 802.1X (EAP-TLS) | Requires certificate-based authentication; an Evil Twin won't have the correct server certificate, and the client will refuse to connect. |
                | VPN | Encrypts all traffic, making intercepted data unreadable to the attacker. |
                | WIPS | A Wireless Intrusion Prevention System can detect and automatically disable unauthorized APs or duplicate SSIDs. |
                | HTTPS | Ensures end-to-end encryption between the browser and the server, protecting data even on an insecure network. |

                Bluejacking vs. Bluesnarfing or Initialization Vector (IV) attacks?
...
-   *Worm*
    ...
    -   2.  Cause disruptions to your normal network traffic since they are constantly trying to replicate and spread themselves across the network
...
-   *Data at Rest*
    -   Data stored in databases, file systems, or storage systems, not actively moving
        -   Encryption Methods
            ...
            **Core Concept:** Cryptographic protocols that provide **Encryption**, **Authentication**, and **Integrity** for data in transit (e.g., HTTPS).
            ---

            ### **1. SSL vs. TLS**
            * **SSL (Secure Sockets Layer):** The predecessor. All versions (1.0, 2.0, 3.0) are now **deprecated** and insecure.
            * **TLS (Transport Layer Security):** The modern standard. 
                * **TLS 1.2:** Widely used; supports older systems but has more overhead.
                * **TLS 1.3:** Faster and more secure (removes weak algorithms, simplifies the handshake).

            ### **2. The "Handshake" Process**
            1.  **Negotiation:** Client and Server agree on the TLS version and **Cipher Suite**.
            2.  **Authentication:** Server sends its **Digital Certificate** (Public Key) to prove identity.
            3.  **Key Exchange:** They use asymmetric encryption to securely agree on a **Symmetric Session Key**.
            4.  **Secure Session:** All subsequent data is encrypted using that symmetric key (faster for bulk data).

            ### **3. Security+ Exam Highlights**
            * **Purpose:** Protects against **Man-in-the-Middle (MitM)** attacks and sniffing.
            * **Ports:** * **HTTPS:** Port 443
                * **FTPS:** Port 990
            * **PFS (Perfect Forward Secrecy):** A feature of modern TLS where a stolen server private key cannot be used to decrypt *past* traffic.
            * **Vulnerability:** **Downgrade Attacks** (forcing a connection to use old SSL/TLS versions) are a common exam topic. **Fix:** Disable SSL and TLS 1.0/1.1.
...
-   *Data Sovereignty*
    ...
    -   Data sovereignty and geographical considerations pose complex challenges, but
        ...
        organizations can navigate them successfully with planning, legal guidance, and strategic technology use, 
...
-   *Geographic Restrictions (Geofencing)*
    ...
    -   *Permission Restrictions*
        ...
        -   **Steganography** is the practice of concealing a file, message, image, or video within another file, message, image, or video. It doesn't use a database. E.g Newspaper spy message

        *Pseudo-anonymization*
        Pseudonymization is a broad term for replacing data with an alias can be restored with a key. 

        Tokenization is a specific form of pseudonymization often used in the payment card industry (PCI DSS) where a credit card number is replaced by a "token" that has no extrinsic value.

        Exam Tip: If a question mentions "permanently removing" or "irreversibly stripping" PII, the answer is Anonymization. 
        If it mentions "masking data that can be restored with a key," the answer is Pseudonymization.
...
-   *Data Loss Prevention (DLP)*
    ...
    OBJ 4.5: Configuring DLP to scan outbound emails and files helps identify and prevent data loss, reducing the risk of breaches while supporting legitimate business needs. Blocking all email attachments and USB devices could limit data leakage but may disrupt necessary operations. The core value of DLP lies in preventing data loss in real-time, not just monitoring. While internal transfer monitoring can be beneficial, the priority here is preventing sensitive data from leaving via email or removable storage. 
...
-   Algorithms
    ...
    -   *Public Key Infrastructure (PKI)*
        ...
        - What term is used to describe the creation, distribution, storage, and revocation of digital certificates? PKI
    ...
    -   Components of Risk Register
        ...
        -   Risk Tolerance and Risk Appetite
            ...
            -   *Risk Appetite*
                ...
                -   Types
                    -   Expansionary
                        -   willing to take additional risks
                    -   Conservative
                        -  focused on products with predictable returns and markets with stable regulatory environments,
                    -   Neutral
...
-   Types of Contracts and Agreements
    ...
    -   *Business Partnership Agreement (BPA) or Joint Venture Agreement (JV)*
        ...
        OBJ: 5.3 - A **Memorandum of understanding (MOU)** is a formal agreement between two or more parties that outlines their mutual understanding and intentions to collaborate. It serves as a precursor to a legally binding contract and establishes a framework for future negotiations. 
        **A Master Service Agreement (MSA)** is a comprehensive contract that sets forth the general terms and conditions that will govern multiple future engagements between the parties. It may reference specific work orders or statements of work for individual projects.
        ...
        **A Business partners agreement (BPA)** is a contractual agreement between two business entities that outlines their collaborative efforts, roles, and responsibilities in a specific business venture or partnership

        OBJ 5.3 - **A Service Level Agreement (SLA)** defines the expectations between an organization and a third-party provider, specifying performance metrics, confidentiality, and security requirements. 
        An **Memorandum of Understanding (MOU)** is less formal, a 
        **Business Partnership Agreement (BPA)** governs business partnerships, and a 
...
-   *Organizational Governance*
    ...
    -   **Advisory Councils** may also provide advice, they are not solely responsible for in-depth analysis and recommendations for the governance board. 
...
-   Incident response procedures
    -   Objective is to identify gaps in security policies and controls for compliance with regulatory standards such as
            -   GDPR
                - OBJ: 5.1 - **GDPR** mandates that personal data can only be collected and processed with the individual's **informed consent**
            -   HIPAA
            -   PCI DSS
    ...
    -   Independent Third-Party Audits
        ...
        - PCI DSS is for credit card data; GDPR is the mandatory data privacy regulation for any business operating in Europe.
...
-   *Data Backup*
    ...
    -   Backup types, organized by how they handle data and their impact on recovery time.

        ---

        ## 1. Full Backup
        The foundation of all other backup types. It is a complete copy of all selected data.
        * **Pros:** Fastest and simplest recovery (only one set of data to restore).
        * **Cons:** Takes the longest to perform and requires the most storage space.
        * **Risk:** As you noted, if you only do this weekly, you risk losing everything since the last "Full" run.

        ## 2. Differential Backup
        Captures all data that has changed **since the last Full backup**.
        * **How it works:** If you do a Full backup Sunday, Monday’s differential contains Monday's changes. Tuesday’s differential contains **Monday AND Tuesday's** changes.
        * **Restoration:** Requires **2 sets**: The last Full backup + the most recent Differential.
        * **Risk:** Potential loss of data since the last differential was run (usually 24 hours).

        ## 3. Incremental Backup
        Captures only the data that has changed **since the last backup of any kind** (Full or Incremental).
        * **How it works:** Monday’s incremental has Monday's changes. Tuesday’s incremental **only** has Tuesday's changes.
        * **Restoration:** Requires **multiple sets**: The last Full backup + **every single incremental** since then, in order.
        * **Risk:** Lowest storage use, but the longest recovery time. If one tape/file in the chain is corrupted, the rest are often useless.

        ## 4. Continuous Data Protection (CDP)
        Also known as **Real-time Backup**. It automatically saves a copy of every change made to the data.
        * **How it works:** It acts like a journal, recording every write folder/file change.
        * **Pros:** Near-zero RPO (Recovery Point Objective). You can "roll back" to seconds before a crash.
        * **Best for:** High-transaction databases or critical financial systems.
        ---

        ## Backup Comparison Summary

        | Backup Type | Space Required | Backup Speed | Restore Speed | Files Needed for Restore |
        | :--- | :--- | :--- | :--- | :--- |
        | **Full** | High | Slow | Fastest | 1 (The Full) |
        | **Differential** | Medium | Medium | Fast | 2 (Full + Latest Diff) |
        | **Incremental** | Low | Fastest | Slowest | Many (Full + All Incs) |
        | **Continuous** | Very High | Constant | Instant-ish | Journal/Log |

        ---

        ### A Quick Tip for your Exam:
        A common "trick" question asks which method is best for a company with a **short backup window** (limited time at night to run backups) versus a **short recovery window** (needs to be back up and running immediately).
        * Short **Backup** Window $\rightarrow$ **Incremental** (it's fast to run).
        * Short **Recovery** Window $\rightarrow$ **Full or Differential** (fewer files to process).
    ...
    -   *Snapshots*
        ...
        -   Snapshots provide a way to capture the system's state at a specific moment in time. They can be used for quick recoveries but don't track continuous transactional changes like journaling.
    -   Use cases
...
-   Monitoring
    ...
    -   Software-Defined Wide Area Networks (SD-WAN) and Secure Access Service Edge (SASE)
        -   *SD-WAN*
            ...
            - SD-WAN (Software-defined wide area network) provides centralized network management, flexible routing, and traffic management capabilities
        -   *SASE*
...
-   *Screened Subnet (Dual-homed Host)*
    ...
    -   Layer based Firewalls
        ...
        -   *Layer 7 Firewall*
            ...
            -   Tracking of connections and requests, allowing return traffic for outbound requests, and improving awareness of connection states on layer 4 were already features on the stateful firewall that was replaced.
            -   The addition of multiple functions, including firewall, intrusion prevention, antivirus, and more, is advancing into the area of a Unified Threat Management (UTM) firewall rather than a basic NGFW. 
            -   Increased focus on HTTP traffic helping to prevent common web application attacks like cross-site scripting and SQL injections would be a description of a Web Application Firewall (WAF), not a NFGW.

            OBJ 4.5: Allowing any outgoing traffic to any destination is NOT recommended for enhancing security. Limiting unrestricted outbound traffic is crucial for security, as it may carry sensitive data and allow unauthorized communication with potentially harmful external servers. Controlling outbound traffic helps prevent data leaks and ensures only necessary communication is permitted. 
            - Enabling stateful inspection in firewalls is recommended, as it tracks active connections and allows only legitimate packets, thereby blocking unauthorized traffic. 
            - Closing unused ports and protocols further reduces the attack surface, preventing threats from exploiting open ports. Although port forwarding can allow remote access to internal servers, it should be used cautiously and only for essential services to avoid introducing security risks. 
...
-   Key difference
    ...
    -   *Intrusion Detection Systems (IDS)*
        -   An IDS (Intrusion Detection System) is specially designed to monitor network traffic, detect potential security incidents, and send alerts
...
-   *Port Security*
    ...
    -   Port security, a feature of managed network switches, allows an organization to limit which devices can connect to a network based on their physical addresses (MAC addresses)
    ...
    -   *EAP (Extensible Authentication Protocol)*
        ...
        -   Extensible Authentication Protocol (EAP) is a network access authentication protocol that can handle multiple authentication methods.
            -   *EAP-MD5*
    ...
    OBJ 3.2: EAP (Extensible Authentication Protocol) provides a standard interface for integrating multiple authentication methods. It can be used in various network access scenarios, including wireless, to provide flexible authentication without requiring changes to the underlying authentication mechanism. 
    -   WPA3 (Wi-Fi protected access 3) is a security protocol designed for securing wireless networks. While it incorporates authentication mechanisms, it isn't as flexible or extensible in terms of supporting multiple authentication methods as EAP. 
    -   LDAP (Lightweight directory access protocol) is a protocol used to access directory systems over IP networks. It is mainly used for user directory querying and authentication, not specifically for the flexibility in wireless or wired network authentication methods. 
    -   RADIUS (Remote authentication dial-in user service) is a protocol for carrying authentication, authorization, and configuration information between a network access server and a central server. While it can support EAP as one of its methods, it is not an authentication method by itself.

    When you are securing a network, you’ll often run into these three protocols. While they all play a part in **Authentication, Authorization, and Accounting (AAA)**, they operate at different layers and serve different purposes.

    Think of **RADIUS** and **TACACS+** as the "security guards" at the server backend, while **EAP** is the "handshake" used to pass your ID badge to those guards.
    ---

    ## 1. RADIUS (Remote Authentication Dial-In User Service)
    RADIUS is the most common protocol used for network access. It is an open standard, making it highly compatible across different hardware brands.

    * **How it works:** It combines **Authentication** and **Authorization** into a single step. When you log in, the server confirms who you are and what you can do at the same time.
    * **Security:** Only the **password** is encrypted. The username and other session information are sent in cleartext.
    * **Transport:** Uses **UDP**, which is faster but less reliable than TCP.
    * **Best for:** ISP connections, 802.1X (Wi-Fi) security, and general network access.

    ## 2. TACACS+ (Terminal Access Controller Access-Control System Plus)
    Developed by Cisco, TACACS+ is designed specifically for **device administration** (managing routers and switches).

    * **How it works:** It strictly separates **Authentication, Authorization, and Accounting**. This allows a granular level of control—for example, you can allow a user to log into a router (Authentication) but forbid them from running specific commands like `reload` (Authorization).
    * **Security:** The **entire packet** is encrypted, making it more secure for internal administrative traffic.
    * **Transport:** Uses **TCP**, which ensures the management commands are delivered reliably.
    * **Best for:** Controlling what IT admins can do on network infrastructure.



    ## 3. EAP (Extensible Authentication Protocol)
    EAP is **not** a standalone AAA protocol like the other two. Instead, it is a **framework** or a "wrapper" used to transport different authentication methods.

    * **How it works:** EAP provides a standard way for a client (like your laptop) to talk to an authenticator (like a Wi-Fi Access Point). The Access Point then wraps that EAP message into a RADIUS packet to send to the backend server.
    * **Variations:** You’ve likely seen EAP-TLS (using certificates) or PEAP (using passwords).
    * **Best for:** Wireless networks and Point-to-Point (PPP) connections.

    ---

    ## Quick Comparison Table

    | Feature | RADIUS | TACACS+ | EAP |
    | :--- | :--- | :--- | :--- |
    | **Primary Use** | Network Access (Users) | Device Admin (IT Staff) | Auth Framework (Wi-Fi) |
    | **Protocol Type** | AAA Protocol | AAA Protocol | Authentication Framework |
    | **Transport** | UDP | TCP | Layer 2 (Data Link) |
    | **Encryption** | Password only | Entire Packet | Depends on the EAP type |
    | **Standard** | Open Standard (RFC) | Cisco Proprietary (mostly) | Open Standard (IEEE) |

    ### In Short:
    * Use **RADIUS** to let users onto the network.
    * Use **TACACS+** to control what admins do on the routers.
    * Use **EAP** as the language the client speaks to the RADIUS server to provide credentials.
    ...
    -   Security Zones and Screened Subnets
        ...
        -   *Screened Subnets*
            ...
            -   Use the term "screened subnet" instead of "DMZ" for modern configurations
                ...
                A DMZ (demilitarized zone) is a physical or logical subnetwork that acts as a secure buffer between a private internal network and the public internet. It hosts public-facing services—such as web, email, and DNS servers—isolating them to prevent an attack on these systems from compromising the internal network.
...
-   *Federation*
    ...
    -   Benefits
        ...
        In federation standards like SAML, the **Identity Provider** (asserts identity) and **Service Provider** (provides the app) are the two core roles.
...
-   Different Types of Access Control Models
        ...
        -   While the **principle of least privilege** ensures minimal access rights, it does not provide short-lived credentials. 
        -   **Static access tokens** are long-lived and don't automatically expire after a short task, posing a higher security risk. 
        - While **time-of-day restrictions** limit access based on specific times, it doesn't ensure credentials are temporary.
        ...
        OBJ 4.6: **Ephemeral credentials** are temporary and are typically generated on-the-fly for a specific purpose, reducing the risk of credential misuse or compromise. 
        Password vaulting involves storing passwords securely, usually with encryption, and doesn't directly provide time-bound access. While Public Key Infrastructure (PKI) provides a framework for secure communications and digital signatures, it does not inherently offer temporary or short-lived credentials. 
...
-   *XML Injection*
    ...
    -   *Race Conditions*
        ...
        -   Key Terms
            ...
            -   Time-of-Use (TOU)
    -    A **TOCTOU attack** exploits a race condition that occurs when a process checks the state or value of a resource before using it, leaving a small gap between the check and when the actual use of the resource takes place. If an attacker can make the change during the gap, this attack can lead to incorrect or unauthorized actions based on outdated information. It can lead to incorrect or unauthorized actions based on invalid assumptions.
...
-   Domain Hijacking
    ...
    -   *Indicators of Compromise (IoC)*
        -   Examples
            ...
            -   Impossible travel is an indicator of malicious activity that shows that an attacker or malware has used an account from two different locations or devices within a short time span, indicating a possible compromise or impersonation. 
...
To understand the difference between these two, it helps to first realize that they are often used together. Most modern DDoS attacks are both **reflected** and **amplified** to maximize the damage to the target.

### **The Core Differences**

| Feature | Reflected DDoS | Amplified DDoS |
| :--- | :--- | :--- |
| **Primary Goal** | To hide the attacker's true IP address. | To multiply the volume of traffic sent to the target. |
| **The Mechanism** | Uses a third-party server (reflector) to "bounce" a request to the victim. | Exploits protocols where a small request triggers a very large response. |
| **Dependency** | Relies on **IP Spoofing** (pretending to be the victim). | Relies on a high **Bandwidth Amplification Factor (BAF)**. |
| **Analogy** | Sending a letter with a fake return address so the recipient gets the reply. | Ordering a giant "family meal" to be delivered to someone else's house using a small coupon. |

---

### **1. Reflected DDoS Attack**
In a reflected attack, the attacker does not send traffic directly to the victim. Instead, they send requests to legitimate third-party servers (like DNS, NTP, or Web servers). 

* **The Trick:** The attacker "spoofs" the source IP address. They tell the third-party server, *"Hi, I'm the victim. Please send the answer to my IP address."*
* **The Result:** The third-party server sends the response to the victim. The victim is flooded with "replies" for requests they never made, and the attacker’s identity remains hidden behind the legitimate server.



### **2. Amplified DDoS Attack**
An amplified attack focuses on the **size** of the traffic. The goal is to turn a small amount of attacker bandwidth into a massive flood that crashes the victim's network.

* **The Trick:** The attacker targets protocols (like DNS or NTP) where the response is significantly larger than the request.
* **The Ratio:** For example, a 60-byte DNS request might trigger a 3,000-byte response. This is a **50x amplification**. 
* **Memcached Example:** In extreme cases (like Memcached attacks), the amplification factor can be as high as **50,000:1**.


---

### **How They Work Together**
In a **DNS Amplification Attack** (the most common type), the attacker combines both:
1.  **Reflection:** They spoof the victim's IP so the DNS servers "reflect" the traffic to the victim.
2.  **Amplification:** They ask for a very large DNS record (like a "DNSSEC" record) so the response is massive compared to the tiny request they sent.

### **Exam Tip for SY0-701**
If the exam asks about **UDP-based** attacks, think of Reflection and Amplification. Because UDP is "connectionless," it is much easier to spoof the source IP than it is with TCP (which requires a 3-way handshake).
...
-   Insecure due to insufficient data integrity checks in the TKIP implementation
    ...
    -   *WPA3 (Wi-Fi Protected Access 3)*
        ...
        -   Features
            ...
            OBJ 4.1: WPA3 is the latest and most secure wireless security protocol; 
            -   it utilizes a Diffie-Hellman key agreement; 
            -   it provides individualized data encryption even in open networks; and 
            -   it prevents eavesdropping, forging, and tampering with management frames. 
            These were some of the additions to address flaws that were present in the previous WPA2 standard, which became an increasing security concern. The 4-way handshake was actually a part of the previous WPA2 standard, 
            
            using a pre-shared key (PSK) that was replaced with the Diffie-Hellman key agreement. Enterprise Wi-Fi networks often use EAP-TLS (Extensible Authentication Protocol with TLS) for authentication (such as TACACS+), but this operates in conjunction with WPA3 and is not part of WPA3 itself.
    ...
    -   How EDR Works
        ...
        -   *Threat Detection*
            ...
            -   1. Signature-Based Detection
                This is the traditional "antivirus" approach. The system maintains a dictionary of binary strings or hashes belonging to known malicious files.

                The Pro: It is extremely fast and accurate for known threats. If the hash matches, it's 100% a match.

                The Con: If an attacker changes just one character in the malware's code, the hash changes, and the signature-based system will let it right through.

                2. Behavioral-Based (Anomaly) Detection
                This is the more modern, "AI-driven" approach. It doesn't care what the file looks like; it cares what the file does.

                The Baseline: First, the system watches your network for a week to learn that "Israel usually logs in at 9 AM from DC and uses 50MB of data."

                The Trigger: If "Israel" suddenly logs in at 3 AM from a different IP and tries to download 10GB of data, the system flags it as an anomaly, even if the login credentials are correct.
...
-   Malware signatures
    ...
    This version separates the **Sources** of intelligence from the **Standards** (how it's shared) and the **Analytical Tools** used to interpret it.

    ---

    ## 🛰️ Threat Intelligence (Domain 2: SY0-701)

    ### **1. Understanding Threat Intelligence**
    * **Definition:** The continuous, evidence-based process of analyzing existing or emerging hazards to an organization's assets.
    * **Goal:** Provide actionable context (mechanisms, indicators, and implications) to stay ahead of evolving attack methods.
    * **Evolution:** Shift from **Server-Side** attacks (open ports/protocols) to **Client-Side** attacks (targeting apps, mobile, and cloud).

    ### **2. Intelligence Sources**
    * **OSINT (Open-Source Intelligence):** Publicly available data (reports, forums, social media, SANS Internet Storm Center). Low cost, but high volume.
    * **Proprietary/Third-Party:** Subscription-based commercial feeds (FireEye, CrowdStrike). Offer more refined, timely, and automated data.
    * **Vulnerability Databases:**
        * **CVE (Common Vulnerabilities and Exposures):** Maintained by MITRE; a list of publicly disclosed cybersecurity vulnerabilities.
        * **NVD (National Vulnerability Database):** Maintained by NIST; enhances CVE data with analysis, such as CVSS scores.
    * **Dark Web:** Hidden areas of the internet (Tor) where criminals sell hacking tools and stolen data. Often contains "Zero-Day" info before it hits public databases.
    * **Information Sharing Organizations:**
        * **ISACs (Information Sharing and Analysis Centers):** Industry-specific (e.g., Financial, Healthcare) collaboration.
        * **InfraGard:** Partnership between the FBI and the private sector.

    ### **3. Sharing Standards (The "How")**

    * **STIX (Structured Threat Information eXpression):** An open standard that defines **what** information is shared (the language).
    * **TAXII (Trusted Automated eXchange of Intelligence Information):** An open standard that defines **how** information is shared (the transport protocol).
    * **AIS (Automated Indicator Sharing):** A CISA-led initiative that uses STIX/TAXII to share indicators in real-time.

    ### **4. Indicators & Analysis**
    * **IOCs (Indicators of Compromise):** Forensic evidence of a past or ongoing attack.
        * *Examples:* Malware signatures, malicious IP addresses, specific file hashes, or unusual URLs.
    * **Predictive Analysis:** Using intelligence to anticipate an attacker's next move. Challenging, but vital for proactive defense.
    * **Threat Maps:** Visual representations of attack patterns.
        * *Note:* These are often **replays** or anonymized data, not strictly "live" tactical data.
    * **File/Code Repositories:** Platforms like **GitHub** used to share intelligence tools or curated lists of threat resources (e.g., *Awesome Threat Intelligence*).

    ---

    ### **Exam Cheat Sheet: Key Acronyms**
    | Acronym | Role | Source |
    | :--- | :--- | :--- |
    | **CVE** | List of vulnerabilities | MITRE |
    | **NVD** | Database of vulnerabilities | NIST |
    | **STIX** | Data Format (Language) | OASIS |
    | **TAXII** | Delivery Mechanism | OASIS |
    | **OSINT** | Public Information | Various |



    ### 1. MITRE ATT&CK: The "Behavioral" Framework
    - **Context**: Used to categorize what an attacker does once they are inside a network.
    - **The Matrix**: Focuses on specific Tactics, Techniques, and Procedures (TTPs).
    - **Application**: Used for Threat Hunting and Red Teaming. If a question asks how to identify a specific method an attacker used to move laterally, the answer is usually MITRE ATT&CK.

    A security analyst is analyzing and categorizing threat actors based on real-world events to improve incident response. Which framework or activity are they most likely participating in? MITRE ATT&C

    ### 2. The Diamond Model: The "Relational" Framework
    - **Context**: Used for Intrusion Analysis to understand the "points" of a specific attack.
    - **The Four Points**: Maps the relationship between the Adversary, Capability, Infrastructure, and Victim.
    - **Application**: Used to "pivot" during an investigation. For example, if you find malware (Capability), use the Diamond Model to identify the C2 server (Infrastructure) and the Adversary.

    ### Comparison for the Exam
    To help you distinguish them on a test question, remember this "cheat sheet":

    | Framework | Key Focus | Best used for... |
    | :--- | :--- | :--- |
    | **MITRE ATT&CK** | Actions/Behavior | Identifying *how* they are attacking (TTPs). |
    | **Diamond Model** | Relationships | Connecting the *who, what, where, and how* of an event. |
    | **Cyber Kill Chain** | Linear Stages | Understanding the phases of an attack to "break the chain." |
...
-   *Vulnerability Confirmation*
    ...
    -   *Risk Tolerance*
        ...
        *OWASP*
        -   (Open Web Application Security Project) is recognized as the industry-standard framework for web application security
        CompTIA often uses OWASP in scenario-based questions. You might see a question like:

        "A security administrator needs to implement a standard set of guidelines to protect the company's new web portal from common attacks. Which of the following resources should they use?"

        Correct Answer: OWASP

        Incorrect Distractor: CVE (Common Vulnerabilities and Exposures). Why? CVE tracks specific individual bugs in software (like a bug in Windows), whereas OWASP provides the general categories of web attacks.
...
-   *Vulnerability Response and Remediation*
    ...
    -   Exception and Exemption
        -   *Exception*
            ...
            -   Exceptions in vulnerability response and remediation allow organizations to delay or avoid certain remediation actions due to specific reasons, often with an understanding of the associated risks. 
        -   *Exemption*
...
-   *Digital Forensics*
    ...
    -   Four Main Phases of Digital Forensic Procedures
        ...
        -   Ethical Considerations
         ...
         OBJ 4.8: **E-discovery** focuses on the systematic search of electronic data to identify pieces of evidence that can be produced in legal contexts. While data protection methodologies are essential in digital investigations, they are not the central aspect of e-discovery. Though understanding financial implications can be essential, e-discovery's main goal isn't centered around financial assessments. Tool selection is critical, but e-discovery specifically deals with the production of electronic evidence.

         OBJ 4.8: The utilization of cryptographic hashes ensures that digital evidence remains unchanged, preserving its original state for analysis and potential court presentation. While searching is an essential part of forensics, this task is more associated with the e-discovery process. While reporting is critical, it isn't directly a part of the preservation activity. While it's important to document procedures, this activity is more closely tied to the acquisition phase of an investigation.
...
-   Automating Support Ticket Management
    ...
    -   Automating Support Ticket Creation
        ...
        OBJ 4.7: Ticket creation doesn't impact security team cohesion. Its primary purpose in IT operations centers around managing, tracking, and coordinating tasks, requests, and issues - not hiring or recruitment processes. Ticket creation in IT operations enables efficient tracking and management of issues, requests, or tasks raised by users, which is crucial in automation and orchestration. By logging tasks and progress via tickets, better accountability of IT team performance is achieved. Response times, issue resolution, and team productivity can be measured accurately. Tickets create a channel of communication between IT teams, helping in coordinating tasks and managing workloads efficiently. 
...
To help you prep for the SY0-701 exam, here is a consolidated summary of the security standards and frameworks organized by their application and primary focus.

### **CompTIA Security+ (SY0-701) Frameworks & Standards Summary**

| Category | Standard/Framework | Primary Purpose & Focus | Key Feature |
| :--- | :--- | :--- | :--- |
| **Global & General** | **ISO/IEC 27001** | Information Security Management System (ISMS). | **Auditable** and international. |
| | **ISO/IEC 27002** | Specific security controls. | Detailed "how-to" for ISO 27001. |
| | **NIST CSF** | Risk-based cybersecurity outcomes. | Flexible; common in U.S. |
| | **CIS Controls** | Prioritized technical best practices. | 18 Actionable "Critical" controls. |
| **Regulatory (Mandatory)** | **GDPR** | Data privacy for EU residents. | **Horizontal** (cross-sector) & strict. |
| | **CCPA** | Consumer privacy rights in California. | **Horizontal**; similar to GDPR. |
| | **HIPAA** | Protection of healthcare data (PHI). | **Vertical** (Healthcare industry). |
| | **PCI DSS** | Security for credit card transactions. | Industry-mandated; not a law. |
| | **GLBA** | Financial data protection. | **Vertical** (Financial services). |
| | **SOC 2** | Service provider auditing. | Reports on security, privacy, etc. |
| **Government Specific** | **FISMA** | Security for U.S. Federal agencies. | Mandatory for federal government. |
| | **FedRAMP** | Cloud security for government use. | Standardized for Cloud Providers. |
| | **CMMC** | DoD contractor certification. | Tiered levels for defense supply chain. |
| **Industry Specific** | **HITRUST CSF** | Harmonized healthcare framework. | Combines HIPAA, ISO, and NIST. |
| | **NERC CIP** | Protection of the electrical grid. | Focuses on bulk power systems. |

---

### **Key Concepts for Domain 5**

* **Horizontal vs. Vertical:** * **Horizontal** regulations (like GDPR or CCPA) apply to almost any business regardless of their industry as long as they handle specific types of data. 
    * **Vertical** regulations (like HIPAA or GLBA) apply specifically to a single industry (Healthcare or Finance).
* **Compliance vs. Frameworks:** * **Frameworks** (NIST, ISO) are "best practice" blueprints you *choose* to follow to be secure. 
    * **Compliance/Regulations** (HIPAA, FISMA) are requirements you *must* follow to avoid legal or financial penalties.


Here is the updated MD table for your SY0-701 study notes, focusing on a concise summary of each standard's role.

### **CompTIA Security+ (SY0-701) Frameworks & Standards**

| Category | Standard / Framework | Short Summary |
| :--- | :--- | :--- |
| **Global & General** | **ISO/IEC 27001** | The gold standard for an auditable Information Security Management System (ISMS). |
| | **ISO/IEC 27002** | A specific set of information security controls used to implement ISO 27001. |
| | **NIST CSF** | A flexible, risk-based framework focusing on Identify, Protect, Detect, Respond, and Recover. |
| | **CIS Controls** | A prioritized list of 18 actionable "Critical Security Controls" to stop common attacks. |
| **Regulatory (Mandatory)** | **GDPR** | Strict EU regulation protecting personal data and privacy for all EU residents. |
| | **CCPA** | California law providing data privacy rights and consumer protection (Horizontal). |
| | **HIPAA** | U.S. law protecting the privacy and security of health information (Vertical). |
| | **PCI DSS** | Contractual requirements for anyone handling credit card data (Industry-mandated). |
| | **GLBA** | Requires financial institutions to explain how they share and protect customer data. |
| | **SOC 2** | An auditing procedure to ensure service providers manage data securely. |
| **Government Specific** | **FISMA** | Law requiring U.S. federal agencies to implement information security programs. |
| | **FedRAMP** | Security assessment and authorization for cloud products used by the U.S. government. |
| | **CMMC** | A certification program to verify that DoD contractors have proper security in place. |
| **Industry Specific** | **HITRUST CSF** | A certifiable framework that maps multiple standards specifically for healthcare. |
| | **NERC CIP** | A set of requirements designed to secure the North American power grid. |

---

### **Quick Comparison: Horizontal vs. Vertical**

Understanding the "scope" of these regulations is a common exam point. 

* **Horizontal Standards:** These apply across **all industries**. If you handle a specific type of data (like personal info), you must comply regardless of whether you are a retail shop or a tech giant.
    * *Examples:* GDPR, CCPA.
* **Vertical Standards:** These are **industry-specific**. They apply only to organizations operating within a particular sector.
    * *Examples:* HIPAA (Healthcare), GLBA (Finance), NERC CIP (Utility/Power).
